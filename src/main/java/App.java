...
public class App {
    ...
    private String extractString(Instance stringInstance) {
    	...
    	@SuppressWarnings("unchecked")
		List<String> characterBytesAsStrings = new ArrayList<String>(stringsValueField.getValues());
    	byte [] stringAsBytes = new byte[characterBytesAsStrings.size()];
    	for(int i = 0; i < stringAsBytes.length; i++) {
    		stringAsBytes[i] = (byte)Integer.parseInt(characterBytesAsStrings.get(i));
    	}
